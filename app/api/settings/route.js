...
function admin(user) {
  if (user.role !== "Super Admin") {
    const e = new Error("Super Admin required.");
    e.status = 403;
    throw e;
  }
}
const validRole = (role) =>
  ["Super Admin", "Manager", "Staff", "Viewer"].includes(role);
...
export async function GET() {
  try {
    ...
    const [settings, users, bank, permissions] = await Promise.all([
      settingsMap(),
      query(
        "select id,username,display_name,email,role,active from users order by id",
      ),
      query(
        "select id,name,opening_balance,notes,active from bank_accounts order by id",
      ),
      query("select user_id,permission_key,allowed from user_permissions"),
    ]);
    const permissionMap = {};
    for (const row of permissions.rows) {
      permissionMap[row.user_id] ||= {};
      permissionMap[row.user_id][row.permission_key] = row.allowed;
    }
    return ok({
      settings,
      users: users.rows.map((x) => ({
        ...x,
        permissions: permissionMap[x.id] || null,
      })),
  ...
  try {
    ...
    if (body.kind === "settings") {
      ...
      for (const [key, raw] of Object.entries(body.values || {})) {
        ...
        if (key === "company_logo" && value.length > 1400000)
          throw new Error("Logo is too large. Use an image under 1 MB.");
        if (
          (key === "company_email" || key === "recovery_email") &&
          value &&
          !/^\S+@\S+\.\S+$/.test(value)
        )
      ...
      await audit(
        ...
        null,
        null,
        "Company and recovery settings updated",
      );
      return ok({ ok: true });
    }
    if (body.kind === "user") {
      ...
      try {
        ...
        if (id) {
          ...
          if (
            ...
            (role !== "Super Admin" || !active)
          ) {
            const count = await client.query(
              "select count(*)::int count from users where role='Super Admin' and active=true and id<>$1",
              [id],
            );
          ...
          await client.query(
            "update users set username=$1,display_name=$2,email=$3,role=$4,active=$5,updated_at=now() where id=$6",
            [username, name, email || null, role, active, id],
          );
          if (password)
            await client.query(
          ...
          const created = await client.query(
            "insert into users(username,display_name,email,role,password_hash,active,created_at,updated_at) values($1,$2,$3,$4,$5,$6,now(),now()) returning id",
            [
              username,
              name,
              email || null,
    ...
    if (body.kind === "bank") {
      await query(
        "update bank_accounts set name=$1,opening_balance=$2,notes=$3,updated_at=now() where id=$4",
        [
          String(body.name || "").trim(),
          Number(body.opening || 0),
          String(body.notes || ""),
          body.id,
        ],
      );
    ...
    return fail(
      duplicate ? "That username is already in use." : e.message,
