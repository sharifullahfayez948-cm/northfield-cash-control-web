...
export async function PATCH(req,{params}){try{
  ...
  await openDay(String(old.business_date).slice(0,10));await ensureDay(date,user.id);await openDay(date);
...
export async function DELETE(req,{params}){try{
  const user=await requireUser();admin(user);const {id}=await params,old=await entry(id);await openDay(String(old.business_date).slice(0,10));
...
export async function POST(req,{params}){try{
  const user=await requireUser();admin(user);const {id}=await params,old=await entry(id),b=await req.json();if(b.action!=="restore")throw new Error("Invalid action.");await openDay(String(old.business_date).slice(0,10));if(old.status!=="VOID")throw new Error("Only voided transactions can be restored.");
