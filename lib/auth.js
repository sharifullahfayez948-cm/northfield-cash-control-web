...
export const PERMISSION_KEYS=["dashboard","transactions","movements","closing","bank","transfer","reports","directory"];
...
export async function accessForUser(user){
  ...
  const r=await query("select permission_key,allowed from user_permissions where user_id=$1",[user.id]);
  if(!r.rows.length)return full;
  return Object.fromEntries(PERMISSION_KEYS.map(k=>[k,r.rows.find(x=>x.permission_key===k)?.allowed===true]));
}
export async function requirePermission(key){
  const user=await requireUser();
  const access=await accessForUser(user);
  if(!access[key]){const e=new Error("ACCESS_DENIED");e.status=403;throw e}
  return user;
}
export async function requireAnyPermission(keys){
  const user=await requireUser();
  const access=await accessForUser(user);
  if(!keys.some(key=>access[key])){const e=new Error("ACCESS_DENIED");e.status=403;throw e}
  return user;
}
