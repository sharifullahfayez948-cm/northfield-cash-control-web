...
export async function ensureDay(date,userId){
  ...
  await audit(userId,"OPEN_DAY","cash_day",null,date,`Opening cash ${opening}`).catch(()=>{});
  return r.rows[0];
}
...
export async function audit(userId,action,entityType,entityId=null,businessDate=null,detail=""){
  ...
  await pushForAudit(action,businessDate,detail).catch(()=>{});
}
